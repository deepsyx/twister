	...
	preFilter: {
		CLASS: function(match, curLoop, inplace, result, not, isXML){
			...
			for ( var i = 0, elem; (elem = curLoop[i]) != null; i++ ) {
				if ( elem ) {
					if ( not ^ (elem.className && (" " + elem.className + " ").replace(/[tn]/g, " ").indexOf(match) >= 0) ) {
						if ( !inplace ) {
							result.push( elem );
						}
					} else if ( inplace ) {
...
for ( var type in Expr.match ) {
	...
	Expr.leftMatch[ type ] = new RegExp( /(^(?:.|r|n)*?)/.source + Expr.match[ type ].source.replace(/\(d+)/g, fescape) );
}
...
var rinlinejQuery = / jQueryd+="(?:d+|null)"/g,
	...
	rchecked = /checkeds*(?:[^=]|=s*.checked.)/i,  // checked="checked" or checked (html5)
	raction = /=([^="'>s]+/)>/g,
	wrapMap = {
...
var jsc = jQuery.now(),
	rscript = /<scriptb[^<]*(?:(?!</script>)<[^<]*)*</script>/gi,
